...
//        {
//            int ntests = 50;
//            int nwoMin = 8, nwoMax = 10;
//            int iwoMax;

//            IdType weekId = new IdType(11);
//            IdType workoutId = new IdType(33);
//            List<WorkoutTemplate> workouts = new List<WorkoutTemplate>();
//            TrainingWeekTypeEnum weekType = TrainingWeekTypeEnum.Generic;

//            // No initial workouts
//            Assert.Throws<TrainingDomainInvariantViolationException>(() => TrainingWeekTemplate.AddTrainingWeekToPlan(weekId, 0, workouts, weekType));

//            // Remove workouts to empty week
//            workouts.Add(StaticUtils.BuildRandomWorkout(workoutId.Id, 0));
//            TrainingWeekTemplate week = TrainingWeekTemplate.AddTrainingWeekToPlan(weekId, 0, workouts, weekType);

//            Assert.Throws<TrainingDomainInvariantViolationException>(() => week.RemoveWorkout(workoutId));
...
//                iwoMax = RandomFieldGenerator.RandomInt(nwoMin, nwoMax);

//                // NULL workouts
//                for (int iwo = 0; iwo < iwoMax; iwo++)
//                {
//                    if (RandomFieldGenerator.RandomDouble(0, 1) < 0.2)
//                        workouts.Add(null);
//                    else
//                        workouts.Add(StaticUtils.BuildRandomWorkout(iwo + 1, iwo));
//                }
//                // Ensure at least one NULL
//                if (workouts.All(x => x != null))
//                    workouts.Add(null);

//                weekType = null;
//                int? weekTypeId = RandomFieldGenerator.RandomIntNullable(0, TrainingWeekTypeEnum.Peak.Id, 0.3f);
...
//                // Full rest + initial workouts
//                if (workouts.All(x => x == null))
//                    workouts = new List<WorkoutTemplate>() { StaticUtils.BuildRandomWorkout(1, 0) };
//                else
//                    workouts = workouts.Where(x => x != null).ToList();

//                weekType = TrainingWeekTypeEnum.FullRest;
//                Assert.Throws<TrainingDomainInvariantViolationException>(() => TrainingWeekTemplate.AddTrainingWeekToPlan(weekId, 0, workouts, weekType));

//                // Full rest + add workout
//                week = TrainingWeekTemplate.AddTrainingWeekToPlan(weekId, 0, new List<WorkoutTemplate>(), weekType);
//                Assert.Throws<TrainingDomainInvariantViolationException>(() => week.AddWorkout(
//                    new List<WorkUnitTemplate>() { StaticUtils.BuildRandomWorkUnit(1, 0), }, "my workout"));
...
//                {
//                    if (RandomFieldGenerator.RandomDouble(0, 1) < 0.2)
//                        workouts.Add(StaticUtils.BuildRandomWorkout(iwo + 1, iwo * 100));
//                    else
//                        workouts.Add(StaticUtils.BuildRandomWorkout(iwo + 1, iwo));
//                }
...
//            // Consecutive pnums but starting from 1
//            workouts = new List<WorkoutTemplate>();

//            iwoMax = RandomFieldGenerator.RandomInt(nwoMin, nwoMax);

//            for (int iwo = 0; iwo < iwoMax; iwo++)
//                workouts.Add(StaticUtils.BuildRandomWorkout(iwo + 1, iwo + 1));
...
//        [Fact]
//        public void TrainingWeekFullTest()
//        {
//            int nWorkoutsMin = 2, nWorkoutsMax = 10;
//            int iwoMax;
...
//            //    // Initial workout - mandatory
//            //    workouts = new List<WorkoutTemplate>()
//            //    {
//            //        StaticUtils.BuildRandomWorkout(1, 0),
//            //    };

//            //    // Add other workouts
//            //    int nWorkouts = RandomFieldGenerator.RandomInt(nWorkoutsMin, nWorkoutsMax);
...
//            //        workouts.Add(StaticUtils.BuildRandomWorkout(strongId, iwo));
//            //    }


//            //    TrainingWeekTemplate week = TrainingWeekTemplate.AddTrainingWeekToPlan(
//            //        id: new IdType(id),
...
//        {
//            int nWeeksMin = 0, nWeeksMax = 8;
//            int nSchedMin = 0, nSchedMax = 3;
//            int nPhaseMin = 0, nPhaseMax = 3;
//            int nProficiencyMin = 0, nProficiencyMax = 3;
//            int nFocusMin = 0, nFocusMax = 3;
//            int nHashtagMin = 0, nHashtagMax = 3;
//            int nChildMin = 0, nChildMax = 3;

//            int messageIdMin = 4000, messageIdMax = 7500;
//            int ownerIdMin = 1, ownerIdMax = 2000;
//            int noteIdMin = 6000, noteIdMax = 9000;

//            IdType planId = new IdType(83);

//            string planName = RandomFieldGenerator.RandomTextValue(4, 20);
//            bool isBookmarked = RandomFieldGenerator.RandomBoolWithProbability();
//            bool isTemplate = RandomFieldGenerator.RandomBoolWithProbability();

//            IdType ownerId = new IdType(RandomFieldGenerator.RandomInt(ownerIdMin, ownerIdMax));
//            IdType noteId = new IdType(RandomFieldGenerator.RandomInt(noteIdMin, noteIdMax));
//            IdType messageId;

//            TrainingPlanTypeEnum weekType = TrainingPlanTypeEnum.From(RandomFieldGenerator.RandomInt(1, TrainingPlanTypeEnum.Inherited.Id));

//            // Initial Weeks
//            int nweeks = RandomFieldGenerator.RandomInt(nWeeksMin, nWeeksMax);
//            List<TrainingWeekTemplate> weeks = new List<TrainingWeekTemplate>();

//            for (int iweek = 0; iweek < nweeks; iweek++)
//                weeks.Add(StaticUtils.BuildRandomTrainingWeek(iweek + 1, iweek));

//            // Initial Schedules
//            List<IdType> scheduleIds = StaticUtils.BuildIdsCollection(nSchedMin, nSchedMax).ToList();

//            // Initial Phases
//            List<IdType> phaseIds = StaticUtils.BuildIdsCollection(nPhaseMin, nPhaseMax).ToList();

//            // Initial Proficiencies
//            List<IdType> proficiencyIds = StaticUtils.BuildIdsCollection(nProficiencyMin, nProficiencyMax).ToList();

//            // Initial Muscles Focuses
//            List<IdType> focusIds = StaticUtils.BuildIdsCollection(nFocusMin, nFocusMax).ToList();

//            // Initial Hashtags
//            List<IdType> hashtagIds = StaticUtils.BuildIdsCollection(nHashtagMin, nHashtagMax).ToList();

//            // Initial Child Plans
//            List<IdType> childIds;

//            if (isTemplate)
//                childIds = StaticUtils.BuildIdsCollection(nChildMin, nChildMax).ToList();
//            else
//                childIds = null;
...
//            else
//                messageId = new IdType(RandomFieldGenerator.RandomInt(messageIdMin, messageIdMax));

//            TrainingPlan plan = TrainingPlan.CreateTrainingPlan(planId, planName, isBookmarked, isTemplate, ownerId, weekType, noteId, messageId,
//                weeks, scheduleIds, phaseIds, proficiencyIds, focusIds, hashtagIds, childIds);
...
//            //TrainingWeekTemplate week1 = TrainingWeekTemplate.AddTrainingWeekToPlan(new IdType(11), 0, null);

//            //WorkoutTemplate woA1 = WorkoutTemplate.PlanWorkout(new IdType(1), 0, new List<WorkUnitTemplate>(), "A", null);

//            IdType excerciseId = new IdType(119);

//            WorkUnitTemplate wuA11 = WorkUnitTemplate.PlanWorkUnit(new IdType(1), 0, excerciseId, null, null, null);
//            WorkingSetTemplate ws1 = WorkingSetTemplate.AddWorkingSet()

//            wuA11.AddWorkingSet(WorkingSetTemplate.AddWorkingSet(null, ));
//            wuA11.AddWorkingSet(WSRepetitionValue.TrackRepetitionSerie(10));
//            wuA11.AddWorkingSet(WSRepetitionValue.TrackRepetitionSerie(8));
//            woA1.AddWorkUnit(wuA11);

//            //woA1.AddWorkUnit(excerciseId, new List<WorkingSetTemplate>());
//            //woA1.AddWorkingSet(new IdType(11), WSRepetitionValue.TrackRepetitionSerie(12));
//            //woA1.AddWorkingSet(new IdType(11), WSRepetitionValue.TrackRepetitionSerie(10));
//            //woA1.AddWorkingSet(new IdType(11), WSRepetitionValue.TrackRepetitionSerie(8));

//            //week1.AddWorkout(woA1);

//            //WorkoutTemplate woB1 = WorkoutTemplate.PlanWorkout(new IdType(1), 0, null, "B", null);
//            //WorkoutTemplate woC1 = WorkoutTemplate.PlanWorkout(new IdType(1), 0, null, "C", null);




//            //TrainingWeekTemplate week2 = TrainingWeekTemplate.AddTrainingWeekToPlan(new IdType(12), 1, null);
